...
//		employeeEntities.stream().forEach(EmployeeEntity::getEmployeeId);
	...
	public EmployeeDTO updateEmployeeDetails(Long id, @Valid EmployeeDTO employeeDto) {
		// TODO Auto-generated method stub
		employeeExists(id);
		EmployeeEntity oldEmployeeEntity = employeeRepository
				.findById(id)
		...
		modelMapper.map(toUpdateEmployeeEntity, oldEmployeeEntity);
		EmployeeEntity savedEmployeeEntity = employeeRepository.save(oldEmployeeEntity);
		return modelMapper.map(savedEmployeeEntity, EmployeeDTO.class);
	}
